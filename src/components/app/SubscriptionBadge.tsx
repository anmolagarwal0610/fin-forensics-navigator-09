...
export function SubscriptionBadge({ tier, expiresAt, isActive = true }: SubscriptionBadgeProps) {
  const tierConfig = {
    ...
    monthly: { label: 'Monthly Tier', className: 'bg-amber-500/15 text-amber-600 dark:text-amber-400' },
    yearly_tier: { label: 'Yearly Tier', className: 'bg-cyan-500/15 text-cyan-600 dark:text-cyan-400' },
    yearly_plan: { label: 'Yearly Plan', className: 'bg-pink-500/15 text-pink-600 dark:text-pink-400' },
  };
